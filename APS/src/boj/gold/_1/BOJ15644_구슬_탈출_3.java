    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < M; j++) {
                ...
                switch (map[i][j]) {
                    case 'R':
                        red = new Marble(i, j, -1, 0, 0);
                        map[i][j] = '.';
                        break;
                    case 'B':
                        blue = new Marble(i, j, -1, 0, 0);
                        map[i][j] = '.';
                        break;
                    case '0':
                        exit = new Marble(i, j, -1, 0, 0);
                        break;
                }
            }
        }
...
//                        System.out.println(nr.times);
    ...
    private static void print(Marble marble) {
        StringBuilder sb = new StringBuilder();
        int history = marble.history, times = marble.times;
        sb.append(times+"\n");
        while (times-- > 0) {
            int cur = history%4;
            switch (cur) {
    ...
    private static Marble move(Marble marble, Marble another, int dir) {    // 구슬 이동 메서드
        ...
        int history = marble.history + ( dir * (int) Math.pow(4, (marble.times) ) );
        if (isIn(y, x) && map[y][x] == 'O') {   // 구슬이 탈출할 경우
            return new Marble(-1, -1, -1, marble.times + 1, history);
        } else {    // 구슬이 탈출하지 못한 경우
            return new Marble(y + delta[(dir+2)%4][0], x + delta[(dir+2)%4][1], dir, marble.times + 1, history);
        }
    }
    ...
    static class Marble {   // 구슬의 좌표, 방향, 이동횟수를 담은 클래스
        int y, x, dir, times, history;
        public Marble(int y, int x, int dir, int times, int history) {
            this.y = y;
            this.x = x;
            this.dir = dir;
            this.times = times;
